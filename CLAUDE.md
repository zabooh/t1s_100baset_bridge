...
| `bridge_gui.py` | **Bedien-GUI** (tkinter): Bridge-Parameter, alle 183 LAN8651-Register mit Bitfeldern, Testmodi, Terminal. **Standalone** — braucht nur `pyserial`, `lan8651_model.json` und `bridge_config.json`, ruft weder `cli.py` noch `test_lan8651.py` auf (siehe `FALLSTRICKE.md`) |
...
| `check_gui_language.py` | Prüft, dass **alle sichtbaren Texte** in `bridge_gui.py` und `gui_term.py` englisch sind — über den Syntaxbaum, damit Kommentare unberührt bleiben |
