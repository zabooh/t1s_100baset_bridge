...
| `firmware\src\port_mirror.c` `.h` | **Port-Mirror/SPAN** `eth0` → `eth1` — Kommando `mirror`. **Nicht** frei portierbar: braucht den TCP/IP-Stack, `DRV_GMAC_PacketTx` und den gepatchten LAN865x-Treiber (siehe `FALLSTRICKE.md`) |
...
| `bridge_gui.py` | **Bedien-GUI** (tkinter): Bridge-Parameter, alle 183 LAN8651-Register mit Bitfeldern, Testmodi, Terminal, dazu Flash/Erase über die EDBG-Sonde (SWD, unabhängig vom COM-Port) — „Flash from release/" spielt `release/*.hex` über `flash_same54.py` auf eine per Dialog gewählte Sonde, „Erase chip..." löscht Firmware **und** EEPROM-Environment komplett und verlangt dafür ein eingetipptes Bestätigungswort (`ERASE_CONFIRM_WORD`). **Standalone** — braucht nur `pyserial`, `lan8651_model.json` und `bridge_config.json`, ruft weder `cli.py` noch `test_lan8651.py` auf (siehe `FALLSTRICKE.md`) |
| `bridge_gui_modern.py` / `run_gui_modern.bat` | **Dieselbe GUI, sv-ttk-Theme (Windows-11-Fluent-Optik) statt Standard-ttk** — ein Vergleichsbau, kein Ersatz. `BridgeGUIModern(BridgeGUI)` ergänzt nur Theme + eine Farb-Wiederherstellung; serielle Kommunikation, Kommando-Parsing und die Modelle bleiben von `bridge_gui.py` geerbt, unverändert. Braucht zusätzlich `sv-ttk` (`requirements.txt`). sv-ttk löscht beim Aktivieren die roten/grünen Warn-/Erfolgsfarben aus `bridge_gui.py` (Errata-Warnungen, dekodierte Bitfeld-Werte) — `_restore_semantic_colors()` baut sie anhand des Label-Texts wieder auf, siehe `FALLSTRICKE.md` für die Mechanik |
| `gui_term.py` / `term.bat` | **Drei serielle Konsolen in einem Fenster**, ein Klick verbindet alle — Portierung aus dem `t1s_ptp_bridge`-Schwesterprojekt, ohne dessen Sonden-Seriennummer-Auflösung. Welcher COM-Port zu welchem der drei Terminal-Slots gehört, steht in `term_ports.json` (gitignored) und wird über das Menü „Setup → Configure Ports" im Tool selbst gepflegt, nicht von Hand. **Standalone** wie `bridge_gui.py` — nur `pyserial` |
| `gui_term_modern.py` / `term_modern.bat` | **Dasselbe Terminal-Tool, sv-ttk-Theme** — Vergleichsbau wie `bridge_gui_modern.py`, `gui_term.py` bleibt unverändert. `AppModern(App)` erbt Verbindungslogik, Screen-Modell und den Setup-Dialog unveraendert; ergänzt Theme, Titelbalken-Dunkelmodus und die Wiederherstellung der (auch vom Nutzer über „Display" gewählten!) Pane-Terminalfarbe und der Verbindungs-Punkt-Farbe, die sv-ttk sonst beim Start überschreibt. Ein später (z. B. per Setup-Dialog) geöffnetes Fenster ist **nicht** betroffen — sv-ttks Idle-Task-Neufärbung läuft nur einmal, vor dem ersten `root.update()` |
...
| `check_gui_language.py` | Prüft, dass **alle sichtbaren Texte** in `bridge_gui.py`, `bridge_gui_modern.py`, `gui_term.py`, `gui_term_modern.py` und `dep_check.py` englisch sind — über den Syntaxbaum, damit Kommentare unberührt bleiben |
| `dep_check.py` | **Von allen vier GUI-Tools aus `main()` aufgerufen**, bevor irgendetwas gebaut wird — prüft mit `importlib.util.find_spec` (kein echter Import), ob `pyserial`/`sv-ttk` fehlen, und bietet bei einer Lücke einen Tk-Dialog mit „Install now" an (führt `install_dependencies.bat` aus, Ausgabe live gestreamt). `sv-ttk` ist für die `_modern`-Varianten **hart** (kein Weiterlaufen ohne), `pyserial` überall nur **optional** (Tool bleibt nutzbar, nur ohne COM-Port). Grund: siehe `FALLSTRICKE.md`, 2026-08-26 |
...
| `lan8651_model.json` | **Das Registermodell** — 183 Register, 538 Bitfelder, je mit Abschnitt und Seite im Datenblatt, dazu Errata-Anmerkungen sowie Access/Reset je Bitfeld. Die GUI **liest** es und schreibt es nie. Fehler werden **hier** korrigiert, nicht im Python-Quelltext, danach `python check_register_model.py` |
