...
/* Action id used when a raw frame is sent from the PTP trigger.  1 and 2 are the
   trigger module's own demo actions. */
#define NOIP_TRIG_ACTION_ID  3u

/* Send exactly ONE frame, without console output - safe to call from the
   deferred trigger path.  Returns false if the driver refused it. */
bool NOIP_SendOne(void);

/* Registers NOIP_TRIG_ACTION_ID with the trigger module.  MUST be called after
   PTP_TRIG_Initialize(), which clears the action table. */
void NOIP_TrigRegister(void);

/* Sends refused on the trigger path, cumulative. */
uint32_t NOIP_TrigFailCount(void);

/* Is the per-frame RX log enabled?  Off for rate tests, see noip_stat log. */
bool NOIP_RxLogEnabled(void);
