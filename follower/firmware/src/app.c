...
void APP_Initialize(void) {
    ...
    PTP_TRIG_Initialize();
    PTP_TRIG_CliInit();
    /* After PTP_TRIG_Initialize(), which clears the action table: action 3 sends
     * one raw frame, so two boards can transmit on the same absolute instant
     * (duplicate-node-id test, PLCA_BOOTSTRAP_TESTS.md T4a). */
    NOIP_TrigRegister();
    /* 1PPS from the LAN8651 on PC12 (hardware mod, LAN8651_1PPS_HARDWARE.md).
     * Counts edges from the start; feeding the timebase is off until "tbase pps on". */
    PPS_Initialize();
    /* Cross-measurement: the neighbour board's trigger edge on PD11 (EXT1
     * pin 6), timestamped in hardware.  AFTER PPS_Initialize(), because
     * `tbase sec` reads the second edge from there - and after
     * PTP_TRIG_Initialize(), because the grid reference comes from there.
     * Costs no interrupt load (PEER_CAPTURE_PLAN.md 4), so capture is on
     * from the start. */
    PEER_Initialize();
    /* Trigger commands from the master over UDP 30509 (PTP_TIMEBASE_PLAN.md
     * phase D).  The socket opens later, once the stack is up. */
    TRIGCMD_Initialize();
    /* Buttons SW1/SW2 (PD00/PD01), dated on the master's time.  AFTER
     * TRIGCMD_Initialize(), because reporting goes there - and it is the
     * only message this board sends unsolicited.  The two EXTINTs are
     * claimed here and never reconfigured again: a write to EIC_CONFIG
     * briefly takes the instance offline and hits the 1PPS too. */
    BTN_Initialize();
    /* Grid divider: TC6 counts the grid events and flips LED1 every Nth
     * one, with no CPU at all - the same chain as PD10, just with a divider
     * in front.  AFTER PTP_TRIG_Initialize(), because the channel hangs off
     * its generator, and after PIN_Initialize() (the trigger does that),
     * because LED1 comes from the pin table.  The divider is OFF
     * afterwards. */
    GDIV_Initialize();
    /* Endpoint server: the LAN866x protocol (service 0xFF10) on udp/6800,
     * so third-party, unmodified tools can operate this follower.  The
     * socket opens later, once the stack is up - exactly like above. */
    EPSRV_Initialize();

    /* LISTEN FROM RESET.  Following is what a follower is for, so having to type
     * "ptpf on" after every restart was a leftover from when this was a
     * diagnostic switch - and a trap: a board that has not been told to listen
     * looks exactly like one whose timebase has not converged yet, and both make
     * the master's commands fail with NO TIMEBASE.
     *
     * Safe to call here: PTP_FOL_Start() only sets state, and the register work
     * (FTSE/FTSS) is done later by the state machine from PTP_FOL_Tasks().
     * The pin and the hardware firing path are already armed by
     * PTP_TRIG_Initialize(), and the PTP source is the default, so this one call
     * completes the "ready out of the box" state.  It can still be switched at
     * runtime, locally with 'ptpf off' or from the master with 'trigmode'. */
    (void)PTP_FOL_Start();
...
void APP_Tasks(void) {
    ...
    switch (appData.state) {
        ...
        case APP_STATE_IDLE:
        {
            ...
            /* Names the second for a captured 1PPS edge and submits the pair. */
            PPS_Tasks();

            /* Picks up a pending capture of the neighbour edge and computes
             * the phase.  Two register reads per pass, no interrupt - it is
             * SAMPLED, not counted (PEER_CAPTURE_PLAN.md 4). */
            PEER_Tasks();

            /* Trigger commands from the master (UDP 30509).  Arrival time is
             * irrelevant here - the command names an absolute instant. */
            TRIGCMD_Tasks();

            /* Convert and report a pending button press.  ONE event per
             * pass; the timestamp is already fixed by then, so the delay
             * costs no accuracy, only delivery time. */
            BTN_Tasks();

            /* Endpoint protocol: ONE frame per pass, because a handler can
             * arm the trigger and the reply takes buffers from the same
             * pool as the data path. */
            EPSRV_Tasks();
...
bool pktEth0Handler(TCPIP_NET_HANDLE hNet, struct _tag_TCPIP_MAC_PACKET* rxPkt, uint16_t frameType, const void* hParam) {
    ...
    if (NOIP_IsNoIpFrame(frameType)) {
        ...
        /* Counter above always runs; only the per-frame line is gated. */
        if (NOIP_RxLogEnabled()) {
    ...
    /* Bootstrap command channel: raw Ethernet, addressed by MAC.  Taken here and
     * not in the stack, because at this point the sender and this board may well
     * share an IP - which is exactly why the channel exists (trig_someip.h).
     * Acknowledged and consumed: nothing else on the board wants this EtherType,
     * and leaving it to the bridge would flood it to eth1 for no reason. */
    if (frameType == TRIG_ETHERTYPE) {
        TRIGCMD_L2Rx(rxPkt->pMacLayer, rxPkt->pDSeg->segLen);
        TCPIP_PKT_PacketAcknowledge(rxPkt, TCPIP_MAC_PKT_ACK_RX_OK);
        return true;
    }
