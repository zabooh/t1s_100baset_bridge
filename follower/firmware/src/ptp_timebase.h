...
/* Validity of the time this module hands out, in the four levels the LAN866x
 * endpoint protocol uses per timestamp (some_ip.md 8.4).  The mapping is a
 * DECISION, not a rename - see PTP_TB_QualityGet().
 *
 * Why a second scale next to PTP_TB_STATE: the state says whether a MODEL exists,
 * the quality says whether that model is tied to the MASTER.  With `tbase pps on`
 * those two came apart - the 1PPS of this board's own PHY keeps feeding pairs after
 * the grandmaster is gone, so the state stayed LOCKED while the time was no longer
 * common to the fleet.  Measured 2026-08-16: 120 330 trigger edges after `ptp stop`
 * and 121 000 after `testmode 4`, both with `LOCKED / usable yes` and
 * `holdover: entries 0` (test_results.md E26 part 3b).  A follower that reports
 * "certain" while unsynchronised is a lie the moment a foreign host believes it. */
typedef enum
{
    PTP_TB_Q_UNSYNCED = 0,   /* no model at all                                  */
    PTP_TB_Q_UNCERTAIN = 1,  /* a time, but not provably the master's             */
    PTP_TB_Q_CERTAIN = 2,    /* model locked AND the master reference is fresh    */
    PTP_TB_Q_INVALID = 3     /* holdover: do not act on this                      */
} PTP_TB_QUALITY;
...
/* One (local ticks, reference ns) pair.  Called from task context, once per
   ...
   64/64 division. */
/* Where the model's pairs come from.  Only one source feeds it at a time - see
   the comment on s_src in the implementation for why mixing is wrong. */
typedef enum
{
    PTP_TB_SRC_PTP = 0,     /* (L at Sync arrival, t1) - software capture       */
    PTP_TB_SRC_PPS = 1      /* (L at the 1PPS edge, wall clock second) - hardware */
} PTP_TB_SRC;

/* Switching sources clears the model: the reference changes meaning. */
void PTP_TB_SourceSet(PTP_TB_SRC src);
PTP_TB_SRC PTP_TB_SourceGet(void);

/* Submit a pair tagged with its origin; ignored unless that source is active. */
...
/* True when the model is good enough to act on.  Since 2026-08-16 this means
   PTP_TB_Q_CERTAIN, i.e. LOCKED *and* a fresh master reference - not just a
   non-stale model.  The trigger refuses commands unless this holds (plan C.6), and
   stage 0 stops an armed trigger on it, so widening it here is what makes "no
   master -> the pins stop" true with the 1PPS feed as well. */
bool PTP_TB_IsUsable(void);

/* The four-level validity above.  Cheap, no I/O - safe from any context. */
PTP_TB_QUALITY PTP_TB_QualityGet(void);

/* Age of the last PTP pair that ARRIVED and agreed with the running model, in ms.
 * UINT64_MAX while none has.
 *
 * This is deliberately independent of which source FEEDS the model: PTP pairs keep
 * arriving even when 1PPS is the active source (they are dropped by the gate in
 * PTP_TB_SubmitPairFrom, not by the network), so their arrival - and their
 * agreement - is exactly the evidence that a master is present and sane.  A
 * grandmaster restart is caught by the "agreed" half rather than the "arrived"
 * half: pairs keep coming, but t1 jumps back by the master's previous uptime
 * (measured -86 747 s, E26 part 3c). */
/* --------------------------------------------------------------------------- */
/* Trace of the model state - see the long reasoning in ptp_timebase.c          */
/* --------------------------------------------------------------------------- */

/* Output in CHUNKS, not in one go: 256 lines is over a second of line time at
   115200 baud, and a second of blocked main loop is itself a disturbance.
   `from` is the first entry to print, `count` the number of lines; returns
   the next index, or 0 when done. */
uint32_t PTP_TB_TraceDump(uint32_t from, uint32_t count);
uint32_t PTP_TB_TraceCount(void);
void     PTP_TB_TraceSet(bool on);   /* recording on/off; off does not clear  */
