...
/* The intervention cadence is settable at RUNTIME - `ptpf rate <n>`.
 *
 * The reason is a measurement, not convenience: the time base learns from
 * the 1PPS, the 1PPS comes from the PHY clock, and that is exactly what
 * this servo turns.  So the outer loop measures with a ruler the inner one
 * is tugging on, and both run at roughly the same speed (~3.2 s here,
 * ~4 s there).  If the servo intervening less often lowers the edge noise,
 * the interaction is established.
 *
 * On 2026-08-18 the counter-check on the outer loop was already run:
 * `TB_SLOPE_DIV` from 4 to 64 made it 2.7x WORSE (lag), from 4 to 2 nothing
 * significant.  So there is nothing to gain from the outer knob - hence the
 * inner one now.
 *
 * A CAUTION ON INTERPRETING IT: intervening less often also means reacting
 * more sluggishly to real crystal drift.  A better value here is a hint at
 * the cause, not yet the fix - the fix would be to tell the outer
 * controller the known step (feed-forward) instead of leaving it to
 * guess. */
static uint32_t s_rate_every = SERVO_RATE_EVERY;
...
/* Settable at RUNTIME - `ptpf dead <ns>`.  A MAC_TA step shifts the PHY wall
 * clock and thereby the next 1PPS edge; the time base cannot tell that
 * apart from a change in its own crystal.  A larger dead band means the
 * edge is nudged less often, at the price of a static residual left
 * standing up to the dead-band width (constant per board, so harmless to
 * simultaneity as long as it is small). */
static int64_t s_phase_dead_ns = SERVO_PHASE_DEADBAND_NS;
        ...
        case SV_HARDSYNC:
            if (mag <= SERVO_COARSE_NS) {
                ...
                if (s_samples >= s_rate_next_at && s_rate_est_ppb != 0) {
                    s_rate_next_at = s_samples + s_rate_every;
...
static void fol_print_status(void)
{
    ...
    if (!s_servo_on) {
        ...
        SYS_CONSOLE_PRINT("[PTPF] rate cadence: every %lu samples\r\n",
                          (unsigned long)s_rate_every);
...
static void cmd_ptpf(SYS_CMD_DEVICE_NODE *pCmdIO, int argc, char **argv)
{
    ...
    if (argc < 2) {
        fol_print_status();
        SYS_CONSOLE_PRINT("usage: ptpf on | off | status | servo [on|off] | rate [n] | dead [ns] | log [on|off] | reset\r\n");
        return;
    }
    if (!strcmp(argv[1], "on")) {
        if (s_run_requested) {
    ...
    if (!strcmp(argv[1], "rate")) {
        if (argc >= 3) {
            ...
            /* 1 means every sample - that was the state that hunted, per
               the comment above (correction swung 5115 <-> 1897 ppb).  Not
               forbidden, but the bounds stay tight enough that no nonsense
               gets through. */
            if (n < 1 || n > 100000) {
                SYS_CONSOLE_PRINT("[PTPF] rate: expected 1..100000, got %ld\r\n", n);
                return;
            }
            s_rate_every = (uint32_t)n;
            /* Set the next due point to the new cadence at once, or the
               change only takes effect after the old interval - and a
               measurement run starting right afterwards would still
               measure the old state. */
            s_rate_next_at = s_samples + s_rate_every;
        }
        SYS_CONSOLE_PRINT("[PTPF] rate: intervene every %lu samples   (default %u)\r\n",
                          (unsigned long)s_rate_every, SERVO_RATE_EVERY);
        return;
    }
    ...
    if (!strcmp(argv[1], "dead")) {
        if (argc >= 3) {
            long n = strtol(argv[2], NULL, 0);
            /* 0 means 'always jump' - that is the very state the dead band
               is meant to prevent.  Capped above at the COARSE threshold:
               beyond it the phase branch would never run and the servo
               could no longer pay down a real offset. */
            if (n < 0 || n > 100000) {
                SYS_CONSOLE_PRINT("[PTPF] dead: expected 0..100000 ns, got %ld\r\n", n);
                return;
            }
            s_phase_dead_ns = (int64_t)n;
        }
        SYS_CONSOLE_PRINT("[PTPF] dead: %lld ns   (default %lld)   phase jumps so far: %lu\r\n",
                          (long long)s_phase_dead_ns,
                          (long long)SERVO_PHASE_DEADBAND_NS,
                          (unsigned long)s_cnt_steps);
        return;
    }
